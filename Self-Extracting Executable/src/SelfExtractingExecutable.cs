...
namespace SFX
{
    internal class SelfExtractingExecutable
    {
        ...
        public SelfExtractingExecutable()
        {
        ...
        public bool Generate(XElement projXml)
        {
            ...
            {
                Log.LogError(String.Format("Project file ({0}) must contain Name in 'Info-Application'.", projXml.Document?.BaseUri));
                return false;
            }
            ...
            {
                Log.LogError(String.Format("Project file ({0}) must contain SaveFolder in 'Info-SFX'.", projXml.Document?.BaseUri));
                return false;
            }
            ...
            {
                Log.LogError(String.Format("Project file ({0}) must contain a FolderPath in 'Installer' with the installer files.", projXml.Document?.BaseUri));
                return false;
            }
            ...
            {
                Log.LogError(String.Format("Project file ({0}) must contain a MainFileName in 'Installer'.", projXml.Document?.BaseUri));
                return false;
            }
            ...
            {
                Log.LogError(String.Format("The MainFileName in 'Installer' (in {0}) must be located in the directory FolderPath in 'Installer'.", projXml.Document?.BaseUri));
                return false;
            }

            if (!ZipApplicationFiles())
            {
                Log.LogError("Failed to zip application files. Could not run the 7-zip program.");
                return false;
            }
            ...
            {
                Log.LogError("Failed to generate the self-extracting executable.");
                return false;
            }
            ...
            {
                Log.LogWarning("Failed to delete intermediate files.");
            }
        ...
        private bool ZipApplicationFiles()
        {
            ...
            Log.LogInfo("Files (and folders) zipped.");
            return true;
        }
        ...
        private void CreateArchive(string filePath, string runFile, bool showProgressBar, string title)
        {
            ...
            Log.LogInfo("Extraction program generated.");
        }
        ...
        private bool CreateSFX(XElement projXml)
        {
            ...
            Log.LogInfo("Self-extracting executable generated.");
            return true;
        }
    }
}
