...
namespace SFX
{
    internal class ReadProjectData
    {
        ...
        public static string GetProductVersion(XElement projXml)
        {
            ...
            if (version == null)
            {
                Log.LogWarning(string.Format("Project file ({0}) does not contain Version in 'Info-Application'. The product version will be set to 1,0,0,0.", projXml.Document?.BaseUri));
                return "1,0,0";
            }
